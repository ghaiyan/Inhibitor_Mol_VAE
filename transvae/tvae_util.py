...
def clones(module, N):
    """用于克隆N个相同的模块，例如，在构建多头注意力机制时复制相同的注意力头
...
def subsequent_mask(size):
    """
    生成一个掩码来屏蔽未来的位置，主要用于防止解码器在生成当前位置的输出时“看到”未来的位置。
...
def attention(query, key, value, mask=None, dropout=None):
    #实现了缩放点积注意力机制。这是Transformer模型中的关键组件。
...
####### 预处理助手PREPROCESSING HELPERS ##########

def tokenizer(smile):
    # 将SMILES字符串分解为单个符号或原子，这对于模型能够学习如何组合这些符号以生成新分子至关重要。
...
####### 后处理助手POSTPROCESSING HELPERS ##########

def decode_mols(encoded_tensors, org_dict):
    # 将模型输出的编码张量转换回SMILES字符串
...
#######附加度量计算 ADDITIONAL METRIC CALCULATIONS #########
"""
一系列函数用于评估生成的分子的质量，例如通过计算分子权重(calc_MW)、验证生成的SMILES的有效性(valid)、确定唯一性(unique)和新颖性(novel)，以及通过化学指纹计算相似度(fingerprints和tanimoto_similarity)。
"""
def load_gen(path):
...
def pass_through_filters(smiles, data_dir='data'):
    ...
    http://nlp.seas.harvard.edu/2018/04/03/attention.html"""
    _mcf = pd.read_csv(f'{data_dir}/mcf.csv')
    _pains = pd.read_csv(f'{data_dir}/wehi_pains.csv', names=['smarts', 'names'])
