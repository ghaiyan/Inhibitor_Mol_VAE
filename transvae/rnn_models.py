...
class RNNAttn(VAEShell):
    ...
    def __init__(self, params={}, name=None, N=3, d_model=128,
        ...
        Arguments:
            ...
            d_model (int): Dimensionality of model (embeddings and attention)，默认d_model=128,d_model与分子式 encoded_data维度保持一致，此处为224
            d_latent (int): Dimensionality of latent space，默认d_latent=128，d_latent与分子式 encoded_data维度保持一致，此处为224
            dropout (float): Rate of dropout
            d_pp=256,设置为默认d_latent*2
...
class RNNEncoderDecoder(nn.Module):
    ...
    def encode(self, src):
        ...
        #self.src_embed(src).shape: torch.Size([100, 127, 128])
...
class RNNAttnEncoder(nn.Module):
    ...
    def forward(self, x, return_attn=False):
        ...
        #x.shape: torch.Size([100, 127, 128])
        x_out = x_out.permute(1, 0, 2)
        mem = self.norm(x_out)
        """
        mem.shape torch.Size([100, 127, 128])
        attn_weights.shape torch.Size([100, 127, 135])
        """
