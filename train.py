...
def train(args):
    ...
    if args.data_source == 'custom':
        ...
        if args.property_predictor:
            ...
            train_props = pd.read_csv(args.train_props_path, sep='\t', header=None, usecols=[0,1,2,3,4,5,6,7,8]).to_numpy()
            #print(train_props)
            test_props = pd.read_csv(args.test_props_path, sep='\t', header=None, usecols=[0,1,2,3,4,5,6,7,8]).to_numpy()
        else:
            train_props = None
            test_props = None
